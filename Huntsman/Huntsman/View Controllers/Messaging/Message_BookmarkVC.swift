...
protocol Message_BookmarkVCDelegate: class {
    func MessageBookmarkLoaderHide()
    func MessageBookmarkLoaderShow()
}

class Message_BookmarkVC: UIViewController,IndicatorInfoProvider {
    
    var delegate:Message_BookmarkVCDelegate?
    ...
    func callmessagememberListAPI() {
       ...
       self.delegate?.MessageBookmarkLoaderShow()
        let params = [
            URLConstant.Param.NAME:"",
            URLConstant.Param.BOOKMARK:"1"
        ]
         /*USERCHATPOST  Get user chatepost bookmark */
                ...
                if let status = json.value(forKey: "status") as? String {
                    if status == "OK" {
                        ...
                        if totaluser == 0 {
                            ...
                            self.delegate?.MessageBookmarkLoaderHide()
...
extension Message_BookmarkVC : UITableViewDelegate,UITableViewDataSource {
...
extension Message_BookmarkVC : UITextFieldDelegate {
    // Trunk list filter
