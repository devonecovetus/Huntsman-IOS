...
// TrunkShowList_Delegate loder show hide on view
protocol TrunkShowList_Delegate:class {
  
    func Trunkshowlist_LoderShow()
    func Trunkshowlist_LoderHide()
}

class TrunkShow_ListVC: UIViewController, IndicatorInfoProvider {
    
    var Trunks = [TrunkModel]()
    var filteredTrunks = [TrunkModel]()
    
    var delegate:TrunkShowList_Delegate?
    ...
    // MARK: API callGetApptrunk
    func callGetApptrunk() {
        
        self.delegate?.Trunkshowlist_LoderShow()
        let params = [
        ...
        /*  getallTrunk list Updcoming and past trunk list according to Str_Type */
                ...
                if let status = json.value(forKey: "status") as? String {
                    if status == "OK" {
                        ...
                        if TotalTrunk == 0 {
                            ...
                            self.delegate?.Trunkshowlist_LoderHide()
...
extension TrunkShow_ListVC:UITableViewDataSource, UITableViewDelegate, Follow_Bookmark_AttendDelegate, UIScrollViewDelegate, UITextFieldDelegate{
    ...
    @objc func action_attend_unattend(sender: UIButton){
        ...
        if trunk.attend == 0 {
            ...
            let alert = CustomAlert (title:"Huntsman",Message:"You are attending " + trunk.title + ".")
            alert.show(animated: true)
    ...
    @objc func action_bookmark_unbookmark(sender: UIButton){
        ...
        if trunk.bookmark == 0 {
            ...
            self.follow_bookmark_attend.call_bookmarkunbookmark(type: "unbookmark", bookmark_id: trunk.id, bookmark_type: "2", API: URLConstant.API.USER_UNBOOKMARKACTION)
            
            if self.filteredTrunks.count != 0 {
                if let index = self.Trunks.index(where: { $0.id == trunk.id }) {
                    self.Trunks[index].bookmark = 0
                }
                if let index = self.filteredTrunks.index(where: { $0.id == trunk.id }) {
                    self.filteredTrunks[index].bookmark = 0
                }
            }else{
                if let index = self.Trunks.index(where: { $0.id == trunk.id }) {
                    self.Trunks[index].bookmark = 0
