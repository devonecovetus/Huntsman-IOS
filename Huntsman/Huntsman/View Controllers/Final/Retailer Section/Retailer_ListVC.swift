...
protocol Retailer_ListDelegate: class {
    func Retailer_ListLoaderHide()
    func Retailer_ListLoaderShow()
}

class Retailer_ListVC: UIViewController,IndicatorInfoProvider{
    
    var Retailers = [RetailerModel]()
    
    var delegate:Retailer_ListDelegate?
        ...
        WebserviceUtil.callPost(jsonRequest: URLConstant.API.RETAILER_GETALLRETAILER,view: self.view, params: params, success: { (response) in
            ...
            if let json = response as? NSDictionary {
                if let status = json.value(forKey: "status") as? String {
                    if status == "OK"
                    {
                        let totalretailer = json.value(forKey: "total_retailer") as? Int
                        if totalretailer == 0 {
                        
                            self.Retailers.removeAll()
                            
                            self.tableview.reloadData()
                            self.delegate?.Retailer_ListLoaderHide()
...
extension Retailer_ListVC:UITableViewDataSource, UITableViewDelegate, Follow_Bookmark_AttendDelegate, UIScrollViewDelegate, UITextFieldDelegate{
